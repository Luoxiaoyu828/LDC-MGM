...
def get_multi_gauss_params(points_all, params_init):
    ...
    :param params_init: pandas.DataFrame
    ...
    gauss_3d_func = get_multi_gauss_func_by_params(params_init)
    ...
    weight = gauss_3d_value ** power / ((gauss_3d_value ** power).sum())    # 创建拟合的权重
    # weight = np.ones_like(weight)
    errorfunction = lambda p: np.ravel((get_multi_gauss_func_by_params(p)(X, Y, V) - Intensity) * weight)
    # params_fit, success = optimize.leastsq(errorfunction, x0=params_init, )
...
# 对三维高斯云核进行拟合
    ...
    A = np.array([A0, x0, y0, s0_1,s0_2, theta_0, v0, s0_3,A1, x1, y1, s1_1,s1_2, theta_1, v1, s1_3])
    gauss_3d_11 = get_multi_gauss_func_by_params(A)
