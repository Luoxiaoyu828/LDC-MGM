    ...
    while 1:
        ...
        while i < n:
            ...
            if is_join:  # 若第i个能加入，则执行
                ...
                data = res.reshape((size_y, size_x))
                Sum1 = data.sum()
                res_data += data
        ...
        outcat_name = os.path.join(path3_outcat, 'gaussian2D_outcat_%03d.txt' % n_datacube)
        ...
        fits_name = os.path.join(path3_model, 'gaussian2D_model_%03d.fits' % n_datacube)
        ...
        fits_name = os.path.join(path3_out, 'gaussian2D_out_%03d.fits' % n_datacube)
