...
class Header:
    def __init__(self, dim, size, rms, history_info=None, information=None):
        ...
        self.values_3d = [(True,), (-64,), (3,), (size[0],), (size[1],), (size[2],), (-1003.975830078,), (338.0236816406,),
        ...
        self.keys_2d = ['SIMPLE', 'BITPIX', 'NAXIS', 'NAXIS1', 'NAXIS2', 'DATAMIN', 'DATAMAX', 'BUNIT',
                        ...
                        'ALTRPIX', 'RESTFREQ', 'BMAJ', 'BMIN', 'BPA', 'ORIGIN', 'DATE', 'RMS', 'CUNIT3']
        self.values_2d = [(True,), (-64,), (3,), (size[0],), (size[1],), (-1003.975830078,), (338.0236816406,),
                          ('K (T_MB)',), ('GLON-CAR',), (16.75,), (-0.00833333333333,), (181,), (0.0,), ('GLAT-CAR',),
        ...
        if self.dim == 3:
            if self.info is not None:
                for key in self.info.keys():
                    ...
                    self.values_3d[idx] = (self.info[key],)
            for item1, item2 in zip(self.keys_3d, self.values_3d):
                header[item1] = item2
        elif self.dim == 2:
            if self.info is not None:
                for key in self.info.keys():
                    idx = self.keys_2d.index(key)
                    self.values_2d[idx] = (self.info[key],)
            for item1, item2 in zip(self.keys_2d, self.values_2d):
                header[item1] = item2
